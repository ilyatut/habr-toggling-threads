$(document).ready(function(){
	...
	var btn_html = '<span class="btt__switch_btn" style="' + btn_style + '">-</span>';
	$('.comment__head').append(btn_html);

	// create global control buttons
	btn_style += ' margin-top: 4px;';
	var global_btns = 
		'<span class="btt__close_first_level" style="' + btn_style + '">- 1ый</span>' + 
		'<span class="btt__open_first_level" style="' + btn_style + '">+ 1ый</span>' + 
		'<span class="btt__close_all" style="' + btn_style + '">- все</span>' + 
		'<span class="btt__open_all" style="' + btn_style + '">+ все</span>'
	;
	$('.comments-section__subscribe-panel').append(global_btns);
	
	// create open/close event for single comments
	$(document).on('click', '.btt__switch_btn', function(){
	...
	$(document).on('click', '.btt__close_first_level', function(){
	...
	$(document).on('click', '.btt__open_first_level', function(){
		$("#comments-list > .content-list__item_comment").each(function() {
	...
	$(document).on('click', '.btt__close_all', function(){
	...
	$(document).on('click', '.btt__open_all', function(){
		$(".content-list__item_comment").each(function() {
			click_button($(this), false);
		});
	});
	...
	function click_button(element, find_opened) {
		var button = element.find(".btt__switch_btn").filter(':first');
